...
public class CommonMethods {
	...
	public static void scrollWebPage(WebDriver driver, int x, int y) {
		try {
			
			JavascriptExecutor jse1 = (JavascriptExecutor) driver;
			jse1.executeScript("scroll(" + x + ", " + y + ")");
	...
	public void moveToElement_text(String text) throws InterruptedException {
		...
		try {
			...
			e.printStackTrace();
			AllureListener.saveTextLog("Not able to select :  " + text + " from dropdown menu");
			Assert.fail("Not able to select : " + text + " from dropdown menu");

		}
	}

	public static void chooseValueFromDropdown(WebDriver driver, String fieldName, WebElement element,String text) throws InterruptedException {
		
		try
		{
			waitForElement(driver, fieldName, element);
			click_custom(element, fieldName);
	...
	public static void waitForPageLoad(final WebDriver driver, WebElement element) {
		FluentWait<WebDriver> wait = new WebDriverWait(driver, MaxElementWait).pollingEvery(Duration.ofMillis(1000))
	...
	public static String getPropertyValueByKey(String suite, String key) {
		...
		String propFilePath = System.getProperty("user.dir") + "/src/main/resources/usecasedata/" + suite
				+ ".properties";
		FileInputStream fis;
		try {
